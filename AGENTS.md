...
- **依赖**：编译期依赖放 `libs/`（git 忽略）：从 RiaFST 4 mods 目录复制 `ultramarine-1.20.1-0.5.1.jar`、`kaleidoscopecookery-1.2.0-forge+mc1.20.1.jar`（改名为 `kaleidoscopecookery-1.2.0.jar`）、`FarmersDelight-1.20.1-1.2.4.jar`、`kitchenkarrot-1.20.1-0.6.4b.jar`、`culturaldelights-0.16.1.jar`、`chinjufumod-[1.20.1]7.0.1.jar`（改名为 `chinjufumod-7.0.1.jar`，flatDir 不能解析方括号）、`TerraBlender-forge-1.20.1-3.0.1.6.jar`（镇守府硬依赖）、`create-confectionery1.20.1_v1.1.3.jar`（改名为 `create_confectionery-1.1.3.jar`）、`createbigcannons-5.11.1-mc.1.20.1-forge.jar`（改名为 `createbigcannons-5.11.1.jar`）、`ritchiesprojectilelib-2.1.1-mc.1.20.1-forge.jar`（改名为 `ritchiesprojectilelib-2.1.1.jar`，CBC 硬依赖）、`nethersdelight-1.20.1-4.0.jar`、`createaddition-1.20.1-1.3.3.jar`、`Contact-forge-1.2.3.jar`（改名为 `contact-1.2.3.jar`）、`OELib-forge-1.20.1-0.2.4.1.jar`（改名为 `oelib-0.2.4.1.jar`，Contact 硬依赖）。JEI（`jei-1.20.1-forge-15.20.0.127.jar`）、AppleSkin（`appleskin-forge-mc1.20.1-2.5.1.jar`）和 Jade（`Jade-1.20.1-Forge-11.13.3.jar`）仅 `runtimeOnly` 进 `runClient` 调试，不进编译、不声明 mods.toml 依赖。**Create 系**走 `com.simibubi.create:create-1.20.1:6.0.8:slim`：`create-1.20.1-6.0.8.jar` 去掉 `META-INF/jarjar/` 后另存为 `create-1.20.1-6.0.8-slim.jar`，该目录下的四个内嵌 jar 按原名解包进 `libs/` 并由 build.gradle 显式声明——`Ponder-Forge-1.20.1-1.0.91.jar`（catnip 类 `TriState`/`AnimationTickHolder` 所在）、`flywheel-forge-1.20.1-1.0.5.jar`、`Registrate-MC1.20-1.3.3.jar`、`mixinextras-forge-0.4.1.jar`。`gradle.properties` 内置 `systemProp.*.proxyPort=7890` 本地代理。
