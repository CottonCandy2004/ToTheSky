...
- **依赖**：编译期依赖放 `libs/`（git 忽略）：从 RiaFST 4 mods 目录复制 `create-1.20.1-6.0.8.jar`、`FarmersDelight-1.20.1-1.2.4.jar`、`kitchenkarrot-1.20.1-0.6.4b.jar`、`chinjufumod-[1.20.1]7.0.1.jar`（拷到 `libs/` 时改名为 `chinjufumod-7.0.1.jar`，flatDir 不能解析方括号）、`TerraBlender-forge-1.20.1-3.0.1.6.jar`（镇守府硬依赖）、`create-confectionery1.20.1_v1.1.3.jar`（拷到 `libs/` 时改名为 `create_confectionery-1.1.3.jar`）、`createbigcannons-5.11.1-mc.1.20.1-forge.jar`（拷到 `libs/` 时改名为 `createbigcannons-5.11.1.jar`）、`ritchiesprojectilelib-2.1.1-mc.1.20.1-forge.jar`（拷到 `libs/` 时改名为 `ritchiesprojectilelib-2.1.1.jar`，CBC 硬依赖）、`nethersdelight-1.20.1-4.0.jar`、`createaddition-1.20.1-1.3.3.jar`。JEI（`jei-1.20.1-forge-15.20.0.127.jar`）、AppleSkin（`appleskin-forge-mc1.20.1-2.5.1.jar`）和 Jade（`Jade-1.20.1-Forge-11.13.3.jar`）仅 `runtimeOnly` 进 `runClient` 调试，不进编译、不声明 mods.toml 依赖。另从 Create 6 的 `META-INF/jarjar/` 解包 `ponder-forge-1.20.1-1.0.91.jar`（catnip 类 `TriState`/`AnimationTickHolder` 所在）。`gradle.properties` 内置 `systemProp.*.proxyPort=7890` 本地代理。
