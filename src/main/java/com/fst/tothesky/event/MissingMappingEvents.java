 ...
 * <p>
 * 覆盖注册表：方块、物品（含 kjs 方块自动注册的 BlockItem 与流体桶）、流体、状态效果。
...
public final class MissingMappingEvents {
    ...
    private static void remapBlockEntities(MissingMappingsEvent event) {
        for (MissingMappingsEvent.Mapping<net.minecraft.world.level.block.entity.BlockEntityType<?>> mapping
                : event.getMappings(Registries.BLOCK_ENTITY_TYPE, KJS)) {
            String path = mapping.getKey().getPath();
