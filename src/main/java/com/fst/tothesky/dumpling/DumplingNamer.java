...
public final class DumplingNamer {
    ...
    /** 一盘饺子的档案：时长为全部馅料之和的 80%，名称取自随机一份有效馅料 */
    @Nullable
    public static Profile profileForPlate(List<ItemStack> fillings) {
        if (fillings.isEmpty()) {
            return null;
        }
        ...
        for (ItemStack filling : fillings) {
            Profile profile = profileFor(filling);
            if (profile != null) {
                processTicks += profile.processTicks;
            }
        }
        processTicks = (int) Math.floor(processTicks * 0.8);

        Profile nameSource = null;
        for (int attempt = 0; attempt < 80 && nameSource == null; attempt++) {
            nameSource = profileFor(fillings.get(random.nextInt(fillings.size())));
        }
        if (nameSource == null) {
            return null;
        }
